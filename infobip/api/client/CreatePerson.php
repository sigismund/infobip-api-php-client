...
class CreatePerson extends AbstractApiClient
{
     ...
     */
    public function execute(\infobip\api\model\people\Persons $body)
    {
        $restPath = $this->getRestUrl("/people/1/persons");
        $content  = $this->executePOST($restPath, null, $body);
        return $this->map(json_decode($content), get_class(new \infobip\api\model\people\PersonsResponse));
    }
}